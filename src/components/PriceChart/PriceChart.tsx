import { BarList, Bold, Card, Flex, Text, Title } from "@tremor/react";
import { CurrentData } from "../../types";


interface Props {
    currentData: CurrentData;
}

export function PriceChart({ currentData }: Props) {

    const game = currentData.game && currentData.game;
    const gameData = game?.map((g) => ({
        name: g.shop,
        value: g.priceNew,
      }));

    // const GameData = [
    //     {
    //         name: "Steam",
    //         value: 50,
    //         icon: function SteamIcon() {
    //             return (
    //                 <svg
    //                     xmlns="http://www.w3.org/2000/svg"
    //                     className="mr-2.5 fill-orange-500"
    //                     viewBox="0 0 24 24"
    //                     width="20"
    //                     height="20"
    //                 >
    //                     <path fill="none" d="M0 0h24v24H0z" />
    //                     <path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm6.67-10a1.46 1.46 0 0 0-2.47-1 7.12 7.12 0 0 0-3.85-1.23L13 6.65l2.14.45a1 1 0 1 0 .13-.61L12.82 6a.31.31 0 0 0-.37.24l-.74 3.47a7.14 7.14 0 0 0-3.9 1.23 1.46 1.46 0 1 0-1.61 2.39 2.87 2.87 0 0 0 0 .44c0 2.24 2.61 4.06 5.83 4.06s5.83-1.82 5.83-4.06a2.87 2.87 0 0 0 0-.44 1.46 1.46 0 0 0 .81-1.33zm-10 1a1 1 0 1 1 2 0 1 1 0 0 1-2 0zm5.81 2.75a3.84 3.84 0 0 1-2.47.77 3.84 3.84 0 0 1-2.47-.77.27.27 0 0 1 .38-.38A3.27 3.27 0 0 0 12 16a3.28 3.28 0 0 0 2.09-.61.28.28 0 1 1 .39.4v-.04zm-.18-1.71a1 1 0 1 1 1-1 1 1 0 0 1-1.01 1.04l.01-.04z" />
    //                 </svg>
    //             );
    //         }
    //     },
    //     {
    //         name: "GOG",
    //         value: 40,
    //         icon: function RedditIcon() {
    //             return (
    //                 <svg
    //                     xmlns="http://www.w3.org/2000/svg"
    //                     className="mr-2.5 fill-orange-500"
    //                     viewBox="0 0 24 24"
    //                     width="20"
    //                     height="20"
    //                 >
    //                     <path fill="none" d="M0 0h24v24H0z" />
    //                     <path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm6.67-10a1.46 1.46 0 0 0-2.47-1 7.12 7.12 0 0 0-3.85-1.23L13 6.65l2.14.45a1 1 0 1 0 .13-.61L12.82 6a.31.31 0 0 0-.37.24l-.74 3.47a7.14 7.14 0 0 0-3.9 1.23 1.46 1.46 0 1 0-1.61 2.39 2.87 2.87 0 0 0 0 .44c0 2.24 2.61 4.06 5.83 4.06s5.83-1.82 5.83-4.06a2.87 2.87 0 0 0 0-.44 1.46 1.46 0 0 0 .81-1.33zm-10 1a1 1 0 1 1 2 0 1 1 0 0 1-2 0zm5.81 2.75a3.84 3.84 0 0 1-2.47.77 3.84 3.84 0 0 1-2.47-.77.27.27 0 0 1 .38-.38A3.27 3.27 0 0 0 12 16a3.28 3.28 0 0 0 2.09-.61.28.28 0 1 1 .39.4v-.04zm-.18-1.71a1 1 0 1 1 1-1 1 1 0 0 1-1.01 1.04l.01-.04z" />
    //                 </svg>
    //             );
    //         }
    //     },
    //     {
    //         name: "Epic Games Store",
    //         value: 30,
    //         icon: function RedditIcon() {
    //             return (
    //                 <svg
    //                     xmlns="http://www.w3.org/2000/svg"
    //                     className="mr-2.5 fill-orange-500"
    //                     viewBox="0 0 24 24"
    //                     width="20"
    //                     height="20"
    //                 >
    //                     <path fill="none" d="M0 0h24v24H0z" />
    //                     <path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm6.67-10a1.46 1.46 0 0 0-2.47-1 7.12 7.12 0 0 0-3.85-1.23L13 6.65l2.14.45a1 1 0 1 0 .13-.61L12.82 6a.31.31 0 0 0-.37.24l-.74 3.47a7.14 7.14 0 0 0-3.9 1.23 1.46 1.46 0 1 0-1.61 2.39 2.87 2.87 0 0 0 0 .44c0 2.24 2.61 4.06 5.83 4.06s5.83-1.82 5.83-4.06a2.87 2.87 0 0 0 0-.44 1.46 1.46 0 0 0 .81-1.33zm-10 1a1 1 0 1 1 2 0 1 1 0 0 1-2 0zm5.81 2.75a3.84 3.84 0 0 1-2.47.77 3.84 3.84 0 0 1-2.47-.77.27.27 0 0 1 .38-.38A3.27 3.27 0 0 0 12 16a3.28 3.28 0 0 0 2.09-.61.28.28 0 1 1 .39.4v-.04zm-.18-1.71a1 1 0 1 1 1-1 1 1 0 0 1-1.01 1.04l.01-.04z" />
    //                 </svg>
    //             );
    //         }
    //     },
    //     {
    //         name: "Humble Store",
    //         value: 30,
    //         icon: function RedditIcon() {
    //             return (
    //                 <svg
    //                     xmlns="http://www.w3.org/2000/svg"
    //                     className="mr-2.5 fill-orange-500"
    //                     viewBox="0 0 24 24"
    //                     width="20"
    //                     height="20"
    //                 >
    //                     <path fill="none" d="M0 0h24v24H0z" />
    //                     <path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm6.67-10a1.46 1.46 0 0 0-2.47-1 7.12 7.12 0 0 0-3.85-1.23L13 6.65l2.14.45a1 1 0 1 0 .13-.61L12.82 6a.31.31 0 0 0-.37.24l-.74 3.47a7.14 7.14 0 0 0-3.9 1.23 1.46 1.46 0 1 0-1.61 2.39 2.87 2.87 0 0 0 0 .44c0 2.24 2.61 4.06 5.83 4.06s5.83-1.82 5.83-4.06a2.87 2.87 0 0 0 0-.44 1.46 1.46 0 0 0 .81-1.33zm-10 1a1 1 0 1 1 2 0 1 1 0 0 1-2 0zm5.81 2.75a3.84 3.84 0 0 1-2.47.77 3.84 3.84 0 0 1-2.47-.77.27.27 0 0 1 .38-.38A3.27 3.27 0 0 0 12 16a3.28 3.28 0 0 0 2.09-.61.28.28 0 1 1 .39.4v-.04zm-.18-1.71a1 1 0 1 1 1-1 1 1 0 0 1-1.01 1.04l.01-.04z" />
    //                 </svg>
    //             );
    //         }
    //     },
    // ]
    return (
        <>
            <Title className="text-5xl font-bold mt-20 title mb-9">PRICES</Title>
            <Card>
            <h3 className="text-tremor-title text-tremor-content-strong dark:text-dark-tremor-content-strong font-bold mb-3">Data of all stores</h3>

                <Flex>
                    <Text>
                        <Bold>Shop</Bold>
                    </Text>
                    <Text>
                        <Bold>Prices</Bold>
                    </Text>
                </Flex>
                {gameData && <BarList data={gameData} className="mt-2" color="indigo" />}
            </Card>
        </>
    )
}